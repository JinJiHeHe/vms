...
CREATE TABLE user_info(
   `user_id` CHAR(32) NOT NULL AUTO_INCREMENT COMMENT '用户ID/用户名',
   `password` VARCHAR(15) NOT NULL COMMENT '登录密码',
   `phone` CHAR(11) NOT NULL COMMENT '手机号',
   `user_name` VARCHAR(15) NOT NULL COMMENT '真实姓名',
   `birthday` Date NOT NULL COMMENT '生日',
   `gmt_create` DATETIME NOT NULL DEFAULT sysdate() COMMENT '写入时间',
...
CREATE TABLE user_login(
   ...
   `login_time` DATETIME NOT NULL DEFAULT sysdate() COMMENT '登录时间',
